...
const (
	// recommendedHomeDir 定义放置 apis 服务配置的默认目录.
	recommendedHomeDir = ".apis"

	// defaultConfigName 指定了 apis 服务的默认配置文件名.
	defaultConfigName = "configs/apis.yaml"
)
...
func initConfig() {
	...
	// 读取环境变量的前缀为 APIS，如果是 apis，将自动转变为大写。
	viper.SetEnvPrefix("APIS")
	...
	if err := viper.ReadInConfig(); err != nil {
		log.Error("Failed to read viper configuration file", "err", err)
	}
...
func logOptions() *log.Options {
	return &log.Options{
		...
		OutputPaths: []string{"tmp/" + time.Now().Format("2006-01-02") + ".log", "stdout"},
	}
}
...
func jwtOptions() *token.Options {
	return &token.Options{
		Secret:    viper.GetString("jwt.secret"),
		Expire:    viper.GetInt("jwt.expire"),
		TokenId:   viper.GetString("jwt.token-id"),
		BlackPath: viper.GetString("jwt.blacklist-path"),
	}
}
